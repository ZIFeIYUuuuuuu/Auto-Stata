...
| 中文 | English |
| --- | --- |
| 面向实证研究的自动化分析系统，把 Excel 数据转成 Stata 分析和 Word 报告。 | An empirical-research automation system that turns Excel data into Stata analysis and Word reports. |
| 支持中文表头识别、变量角色判断、Stata 脚本生成、模型运行和报告写作。 | Supports Chinese header recognition, variable-role inference, Stata script generation, model execution, and report writing. |
| 适合教学演示、初步分析、项目原型和辅助研究；正式论文仍需人工复核。 | Suitable for teaching demos, first-pass analysis, prototypes, and research assistance; formal papers still need human review. |
...
| RAG hit rate | `N/A` | This project uses structured data analysis, not vector retrieval |
